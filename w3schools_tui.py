...
class LessonScreen(Screen):
    ...
    def on_markdown_link_clicked(self, event: Markdown.LinkClicked) -> None:
        """Handle clicking links in the Markdown viewer."""
        url = event.href
        if url.startswith('/'):
            url = BASE_URL + url
        try:
            # Open URL in Android browser using termux-open
    ...
    async def load_lesson_content(self, rel_url: str):
        try:
            ...
            async with httpx.AsyncClient(headers=HEADERS) as client:
                ...
                if main_content:
                    md_lines = []
                    # Improved parser: preserves <a> links, lists, and code
                    for elem in main_content.find_all(['h1', 'h2', 'h3', 'p', 'pre', 'div', 'ul', 'ol', 'table']):
                        if elem.name.startswith('h'):
                            ...
                            # Convert <a> tags to Markdown [text](href) within paragraphs
                            p_html = str(elem)
                            ...
                            for li in elem.find_all('li'):
                                # Handle links inside list items too
